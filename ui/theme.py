...
def apply_global_theme():
    ...
    base_qss = LIGHT_QSS if theme == 'light' else DARK_QSS
    
    bg_path = APP_CONFIG.get('bg_image', '')
    if bg_path and os.path.exists(bg_path):
        overlay = "rgba(255, 255, 255, 0.85)" if theme == 'light' else "rgba(30, 30, 30, 0.85)"
        base_qss += f"""
            QFrame#MainFrame {{ background-color: transparent; }}
            QWidget#ContentWidget {{ background-color: {overlay}; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px; }}
        """
